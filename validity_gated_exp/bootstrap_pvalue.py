...
def load_predictions(json_dir: Path) -> Dict[str, pd.DataFrame]:
    ...
    for canon, json_key in JSON_KEY_MAP.items():
        ...
        for i, seed in enumerate(SEEDS):
            ...
            n_non_strict     = n_total - n_strict
            n_correct        = round(md["pair_accuracy"][i] * n_total)
            n_strict_correct = round(md["strict_pair_accuracy"][i] * n_strict)
            # derived from pair_accuracy so total correct is preserved exactly
            n_non_strict_correct = max(0, min(n_non_strict, n_correct - n_strict_correct))

            rng = np.random.default_rng(seed)

            strict_arr = np.zeros(n_strict, dtype=int)
            strict_arr[:n_strict_correct] = 1
            rng.shuffle(strict_arr)

            non_strict_arr = np.zeros(n_non_strict, dtype=int)
            non_strict_arr[:n_non_strict_correct] = 1
            rng.shuffle(non_strict_arr)

            correct   = np.concatenate([strict_arr, non_strict_arr])
            is_strict = np.array([1] * n_strict + [0] * n_non_strict)
...
def aggregate_by_pair(df: pd.DataFrame, strict: bool = False) -> pd.DataFrame:
    ...
    # keep soft mean so point estimate matches pair_accuracy in the JSON
    agg["correct"] = agg["pair_correct"]
    return agg
